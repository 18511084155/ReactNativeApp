package com.woodys.demo.reactnative.splashscreen;
...
public class RCTSplashScreen {
        ...
        activity.runOnUiThread(new Runnable() {
            public void run() {
                if(!activity.isFinishing()) {
                    ...
                    dialog = new Dialog(activity, isFullScreen ? android.R.style.Theme_Translucent_NoTitleBar_Fullscreen : android.R.style.Theme_Translucent_NoTitleBar);
    ...
    private static int getImageId() {
        int drawableId = getContext().getResources().getIdentifier("splash", "drawable", getContext().getClass().getPackage().getName());
        if (drawableId == 0) {
            drawableId = getContext().getResources().getIdentifier("splash", "drawable", getContext().getPackageName());
        }
        return drawableId;
    }
