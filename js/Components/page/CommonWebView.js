...
import { ChineseValid } from './../../Util/validate'
...
export default class CommonWebView extends React.Component {
  ...
  _onNavigationStateChange (navState) {
    let title = options.webViewTitle
    if (navState.title) {
      if (ChineseValid(navState.title)) {
  ...
  _getUrl () {
    ...
    let nUrl = `${url}`
    if (url.indexOf('?') === -1) {
      nUrl += `?appName=${options.appName}&appChannel=${options.appChannel}&registerFrom=${options.registerFrom}`
    }
  ...
  render () {
    ...
    return (
      <View style={styles.round}>
        <WebView
          ref='webView'
          source={{uri: this._getUrl(), method: 'post'}}
